<div class="card">
    <div class="card-body">
        ...
        <div class="form-group required {{ $errors->has('trending') ? 'has-error' : '' }}">
            {!! Form::label('card_visibility',trans('payplan.is_trending'), ['class' => 'control-label']) !!}
            <div class="input-group">
                <label>
                    {{ Form::checkbox('trending', '1', isset($payplan)?$payplan->trending:old('trending', false),
                    ['class' => 'icheckblue']) }}
                    {{trans('payplan.trending')}}
                </label>
                <span class="help-block">{{ $errors->first('trending', ':message') }}</span>
            </div>
        </div>
