...
class PaymentController extends Controller
{
    ...
    public function update(Request $request, $payment)
    {
        ...
        $trial_ends_at = now()->gt($payment->trial_ends_at)?now()->addDays(optional($payPlan)->trial_period_days??0)
            :$payment->trial_ends_at->addDays(optional($payPlan)->trial_period_days??0);
        $payment->update([
