    ...
    private function setDbConfigurations()
    {
        ...
        try {
            ...
            if ($driver=='smtp'){
                ...
                config(['mail.encryption' => $settings['mail_encryption'] ?? 'tls']);
            }
            elseif ($driver=='ses'){
                config(['mail.driver' => $driver]);
                config(['services.ses.key' => $settings['ses_key'] ?? null]);
                config(['services.ses.secret' => $settings['ses_secret'] ?? null]);
                config(['services.ses.region' => $settings['ses_region'] ?? null]);
            }elseif ($driver=='mailgun'){
                config(['mail.driver' => $driver]);
                config(['services.mailgun.domain' => $settings['mailgun_domain'] ?? null]);
                config(['services.mailgun.secret' => $settings['mailgun_secret'] ?? null]);
            }elseif ($driver=='elastic_email'){
                config(['mail.driver' => $driver]);
                config(['services.elastic_email.key' => $settings['elastic_key'] ?? null]);
                config(['services.elastic_email.account' => $settings['elastic_account'] ?? null]);
            }else{
                config(['mail.driver' => $driver]);
            }
            config(['mail.from.address'=>$settings['site_email']??null]);
            config(['mail.from.name'=>$settings['site_name']??null]);
